  ...
  const OnSubmitForm = async (data: SubcategorySchema) => {
    ...
    if (subcategoryFromServer) {
      await updateSubCategory.mutateAsync(
        objectToSend as unknown as SubCategory,
        {
          onSuccess: () => {
            goBackToDashBoard();
          },
      ...
      await createNewSubCategory.mutateAsync(
        objectToSend as unknown as SubCategory,
        {
          onSuccess: () => {
            reset();
            navigate(`${subcategoriesUri}/${categoryId}/new`);
          },
        }
      );
    }
        ...
        <Card>
          ...
          <CardContent>
            <Box sx={{ display: "flex", justifyContent: "flex-end", gap: 1 }}>
              {!!subcategoryFromServer && (
                <Button
                  disabled={subcategoryFromServer.hasChildren}
              ...
              <Button
                disabled={
                  createNewSubCategory.isPending || updateSubCategory.isPending
                }
                type="submit"
                variant="contained"
                size="small"
